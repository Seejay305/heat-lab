...
def _make_basic_features(df: pd.DataFrame):
    ...
    df["CLOCK_SEC"] = (
        pd.to_numeric(df["CLOCK_SEC"], errors="coerce").fillna(0).astype(int)
    )
    df["CLOCK_BIN"] = (
        pd.to_numeric(df["CLOCK_BIN"], errors="coerce").fillna(-1).astype(int)
    )
    df["MARGIN_PRE"] = (
        pd.to_numeric(df["MARGIN_PRE"], errors="coerce").fillna(0).astype(int)
    )
    df["MARGIN_POST"] = (
        pd.to_numeric(df["MARGIN_POST"], errors="coerce").fillna(0).astype(int)
    )
...
def main():
    ...
    cal = (
        ...
        .agg(pred=("p", "mean"), emp=("y", "mean"), n=("y", "size"))
        .reset_index()
    )
    ...
    pred = df.loc[
        va_idx, ["GAME_ID", "POSS_SEQ", "EVENTNUM", "PERIOD", "CLOCK_SEC", "EVENT_IDX"]
    ].copy()
