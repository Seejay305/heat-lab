...
def _bootstrap_ci_delta(
    epv_all: np.ndarray, epv_group: np.ndarray, B: int = 500, alpha: float = 0.05
) -> tuple[float, float]:
    """
...
def compute_impacts(
    df: pd.DataFrame, feature: str, min_n: int = 8, B: int = 500
) -> pd.DataFrame:
    """
    ...
    if feature not in df.columns:
        return pd.DataFrame(
            columns=["feature", "value", "delta", "delta_lo", "delta_hi", "n"]
        )
    ...
    # group stats
    gstats = (
        eps.groupby("val", dropna=False)["EPV"].agg(mu="mean", n="size").reset_index()
    )
...
def main():
    ...
    keys = [
        k
        for k in ["GAME_ID", "POSS_SEQ", "EVENTNUM"]
        if k in pred.columns and k in feats.columns
    ]
    if not keys:
