...
def main():
    if not INP.exists():
        raise SystemExit(
            "Missing possessions_enriched.csv. Run: python -m src.features.enrich_possessions"
        )
    ...
    if all(
        ...
        for _, r in df.iterrows():
            ...
            if (
                float(r["DURATION_SEC"]) > 400
            ):  # > 6:40 is suspicious for a single possession
            ...
            if r["RESULT_CLASS"] == "score" and int(r["POINTS"]) == 0:
                add(findings, ps, "RESULT_MISMATCH", "RESULT_CLASS=score but POINTS=0")
            if r["RESULT_CLASS"] == "turnover" and (
                "turnover" not in str(r.get("RESULT", "")).lower()
            ):
                # Soft warning: turnover class without end label turnover
                add(
                    findings,
                    ps,
                    "TURNOVER_LABEL_MISMATCH",
    ...
    if all(c in df.columns for c in ["START_EVENTNUM", "END_EVENTNUM"]):
        ...
        for _, r in bad.iterrows():
            add(
                ...
                r["POSS_SEQ"],
                "EVENTNUM_ORDER",
                f"START_EVENTNUM={r['START_EVENTNUM']} > END_EVENTNUM={r['END_EVENTNUM']}",
            )
    ...
    out_df = (
        ...
        if findings
        else pd.DataFrame(columns=["POSS_SEQ", "TYPE", "DETAIL"])
    )
