...
for name, team in PLAYER_TO_TEAM.items():
    for variant in _name_variants(name):
        ...
        PLAYER_PATTERNS.append(
            (name, re.compile(rf"\b{re.escape(variant)}\b", re.I), team)
        )
...
PLAYER_PATTERNS = [
    (name, re.compile(rf"\b{re.escape(name)}\b", re.I), team)
    for name, team in PLAYER_TO_TEAM.items()
]
...
def enrich(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["TEAM"] = [
        infer_team_from_text(s, e)
        for s, e in zip(out.get("START_TEXT", ""), out.get("END_TEXT", ""))
    ]

    # POINTS from ending event
    out["POINTS"] = [
        points_from_end(res, txt)
        for res, txt in zip(out.get("RESULT", ""), out.get("END_TEXT", ""))
    ]

    # RESULT_CLASS
    out["RESULT_CLASS"] = [
        classify_result(res, pts)
        for res, pts in zip(out.get("RESULT", ""), out["POINTS"])
    ]
    ...
    out["MARGIN_DELTA"] = [
        margin_delta_row(t, p) for t, p in zip(out.get("TEAM", ""), out["POINTS"])
    ]

    # running margin BEFORE and AFTER each possession
    out["MARGIN_PRE"] = (
        out.groupby("GAME_ID")["MARGIN_DELTA"].cumsum().shift(fill_value=0)
    )
...
def main() -> None:
    if not IN_csv.exists():
        raise SystemExit(
            f"Missing {IN_csv}. Run: python -m src.features.run_possessions"
        )
